...
function buildDashboardEmbed(cfg, guild) {
    ...
    return new EmbedBuilder()
        .setTitle('👋 Панель управления системы приветствий')
        .setDescription(
            `Управление настройками приветствия и прощания для **${guild.name}**.\nИспользуйте переключатели, чтобы включить или отключить каждый модуль, а затем выберите вариант для редактирования..`,
        )
        .setColor(getColor('info'))
        .addFields(
            { name: 'Канал Приветствий', value: welcomeChannel, inline: true },
            { name: 'Статус', value: cfg.enabled ? 'Enabled' : 'Disabled', inline: true },
            { name: 'Пинг', value: cfg.welcomePing ? 'On' : 'Off', inline: true },
            { name: 'Канал Прощаний', value: goodbyeChannel, inline: true },
            { name: 'Статус', value: cfg.goodbyeEnabled ? 'Enabled' : 'Disabled', inline: true },
            { name: 'Пинг', value: cfg.goodbyePing ? 'On' : 'Off', inline: true },
            { name: 'Приветственное Сообщение', value: welcomePreview, inline: false },
            { name: 'Прощальное Сообщение    ', value: goodbyePreview, inline: false },
        )
...
function buildSelectMenu(guildId) {
    return new StringSelectMenuBuilder()
        ...
        .setPlaceholder('Выбрать параметр для настройки...')
        .addOptions(
            new StringSelectMenuOptionBuilder()
                .setLabel('Приветственный канал')
                .setDescription('Здесь указывается канал, по которому будут отправляться приветственные сообщения в сообщество')
                .setValue('welcome_channel')
                .setEmoji('🟢'),
            new StringSelectMenuOptionBuilder()
                .setLabel('Приветственное сообщение')
                .setDescription('Здесь изменяется текст, отображаемый при вступлении нового участника в сообщество')
                .setValue('welcome_message')
                .setEmoji('💬'),
            new StringSelectMenuOptionBuilder()
                .setLabel('Приветственная картинка ')
                .setDescription('Здесь устанавливается изображение такое как картинка/гифка для приветственных сообщений')
                .setValue('welcome_image')
                .setEmoji('🖼️'),
            new StringSelectMenuOptionBuilder()
                .setLabel('Прощальный канал')
                .setDescription('Здесь указывается канал, по которому будут отправляться сообщения с прощанием')
                .setValue('goodbye_channel')
                .setEmoji('🔴'),
            new StringSelectMenuOptionBuilder()
                .setLabel('Прощальное сообщение')
                .setDescription('Здесь редактируется текст, отображаемый при выходе участника из сообщества')
                .setValue('goodbye_message')
                .setEmoji('💬'),
            new StringSelectMenuOptionBuilder()
                .setLabel('Прощальная картинка')
                .setDescription('Здесь устанавливается изображение такое как картинка/гифка для прощальных сообщений ')
...
async function handleWelcomeMessage(selectInteraction, rootInteraction, cfg, guildId, client) {
    const modal = new ModalBuilder()
        ...
        .setTitle('Редактирование сообщения Добро пожаловать')
        .addComponents(
            new ActionRowBuilder().addComponents(
...
async function handleWelcomeImage(selectInteraction, rootInteraction, cfg, guildId, client) {
    const modal = new ModalBuilder()
        ...
        .setTitle('Установить изображение на экран приветствия');

    const imageHint = new TextDisplayBuilder()
         .setContent('Укажите прямой URL-адрес на изображение **или** загрузите свой файл ниже. Если указаны оба варианта, приоритет отдается загруженному файлу. Оставьте поле URL пустым , если хотите удалить изображение.');

    const urlLabel = new LabelBuilder()
        .setLabel('URL Ссылка на картинку (Необязательно)')
    ...
    const uploadLabel = new LabelBuilder()
        .setLabel('Загрузить файл с изображением (Необязательно)')
    ...
    await submitted.reply({
        embeds: [successEmbed('Приветственная картинка/гифка загружена', `Image ${imageUrl ? 'updated' : 'removed'} successfully.`)],
        flags: MessageFlags.Ephemeral,
    });
